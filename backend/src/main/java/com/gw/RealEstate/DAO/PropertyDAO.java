...
public class PropertyDAO {
	...
	public void update1(int id, PropertyDetails pd1) {
		...
		for(int i=0;i<listpd.size();i++){
			...
			if(detail.getPropId()==propertydetail.getPropId()){
				...
				propertydetail.setRooms(pd1.getRooms());
				propertydetail.setBathroom(pd1.getBathroom());
				propertydetail.setAddress(pd1.getAddress());
	...
	public void contactForm(Contact contact) {
		...
		System.out.println("add contact detail successfully");
		session.getTransaction().commit();
		session.close();
	}
